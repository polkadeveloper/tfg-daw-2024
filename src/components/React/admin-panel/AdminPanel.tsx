...
function AdminPanel(props: any) {
  ...
  useEffect(() => {
    // Comprueba si el usuario ya se ha autenticado
    const authTime = localStorage.getItem("authTime");
    if (authTime && Date.now() - Number(authTime) < 3600000) {
      // 3600000 ms = 1 hora
      setAuthenticated(true);
    } else {
      // Si ha pasado más de una hora o si no hay un tiempo de autenticación guardado,
      // borra el tiempo de autenticación de localStorage y establece authenticated en false
  ...
  if (!authenticated) {
    return (
      <>
        <section className="min-h-[70vh] px-5 py-2 flex flex-col gap-5 justify-center items-center font-cabinetGroteskBold">
  ...
  if (currentPanel === "create") {
    return <CreateAdminPanel setCurrentPanel={setCurrentPanel} />;
  } else if (currentPanel === "update") {
    return <UpdateAdminPanel setCurrentPanel={setCurrentPanel} />;
  } else if (currentPanel === "delete") {
    return <DeleteAdminPanel setCurrentPanel={setCurrentPanel} />;
  }
